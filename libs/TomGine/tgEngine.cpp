                   ...
                   bool stereo)
{
  ...
  m_input_rotation_speed = 0.2f;
  m_input_translation_speed = 0.2f;
  m_input_zoom_speed = 0.2f;
...
void TomGine::tgEngine::DrawFPS(float interval)
{
}
...
void tgEngine::UpdateCameraViews(tgCamera cam)
{
  ...
  m_cam[1].Orbit(m_cor, m_cam[1].GetU(), float(M_PI));
  m_cam[2].Orbit(m_cor, m_cam[2].GetU(), float(M_PI) * 0.5);
  m_cam[3].Orbit(m_cor, m_cam[3].GetU(), -float(M_PI) * 0.5);
  m_cam[4].Orbit(m_cor, m_cam[4].GetS(), float(M_PI) * 0.5);
  m_cam[5].Orbit(m_cor, m_cam[5].GetS(), -float(M_PI) * 0.5);
}
...
//{
//  Activate2D();
//  g_font->Print(text.c_str(), 15, int(x), int(y), r, g, b, 0.0f);
//}
//
...
//  bool activate2D = m_activate2D;
//  Activate2D();
//  g_font->Print(text.c_str(), size, int(vPos.x), int(vPos.y), r, g, b, a);
//  if (!activate2D)
//    Activate3D();
//}
  ...
  for (int j = 0; j < height; j++)
  {
    for (int i = 0; i < width; i++)
    {
      ...
      if (z_b > dNear && z_b < dFar) // depth range check
      {
        ...
        float z_n = 2.0f * z_b - 1.0f; // normalized depth

        // transform to camera coordinates
        float z = 2.0f * zNear * zFar / (zFar + zNear - z_n * (zFar - zNear));
        float x = (2.0f * float(i) / width - 1.0f + cx) * z / fx;
        float y = (2.0f * float(winY) / height - 1.0f + cy) * z / fy;
