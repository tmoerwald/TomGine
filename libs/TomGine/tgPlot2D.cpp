...
void tgPlot2D::DrawAxis(bool lables, bool grid)
{
  ...
  if( lables )
  {
    ...
    {
      char b[8];
      sprintf_s(b, "%.2f", y_min + ((y_max - y_min) / num_lines_h) * i);
    ...
    {
      char b[8];
      sprintf_s(b, "%.1f", x_min + ((x_max - x_min) / num_lines_v) * i);
...
void tgPlot2D::DrawLegend(int x, int y, std::string y1, std::string y2, std::string y3, std::string y4)
{
  if( !y1.empty() )
  {
