...
tgTextureModel::tgTextureModel() :
  tgRenderModel(), sync(false)  // tgRenderModel::tgRenderModel(), sync(false)
{
}

tgTextureModel::tgTextureModel(const tgTextureModel &m) :
  tgRenderModel(m), sync(false)  // tgRenderModel::tgRenderModel(m), sync(false)
{
  if (!m.m_tex_gl.empty())
  {
      std::cout << "[tgTextureModel::tgTextureModel(const tgTextureModel &)] ";
      std::cout << "Warning, OpenGL Textures cannot be copied in tgTextureModel::m_tex_gl ";
      std::cout << "(" << m.m_tex_gl.size() << ") ";
      std::cout << "'" << m.name.c_str() << "'\n";
  }
...
tgTextureModel::operator=(const tgTextureModel &m)
{
  ...
  if (!m.m_tex_gl.empty())
  {
      std::cout << "[tgTextureModel::operator=(const tgTextureModel &)] ";
      std::cout << "Warning, cannot copy tgTexture2D in tgTextureModel::m_tex_gl ";
      std::cout << "(" << m.m_tex_gl.size() << ")\n";
  }
