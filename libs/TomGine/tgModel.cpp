...
void tgModel::Merge(const tgModel &m)
{
  ...
  for (size_t i = 0; i < m.m_faces.size(); i++)
  {
  ...
  for (size_t i = 0; i < m.m_lines.size(); i++)
    m_lines.push_back(m.m_lines[i]);

  for (size_t i = 0; i < m.m_points.size(); i++)
    m_points.push_back(m.m_points[i]);

  for (size_t i = 0; i < m.m_colorpoints.size(); i++)
...
bool tgModel::CheckFaces() const
{
  for (size_t i = 0; i < m_faces.size(); i++)
  {
    ...
    for (size_t j = 0; j < f.v.size(); j++)
    {
      ...
      {
        std::cout << "[tgModel::CheckFaces()] Vertex index out of bounds (0 <= " << f.v[j] << " < " << m_vertices.size() << ")\n";
        return false;
      }
    }
  }
...
void tgModel::RemoveDublicateVertices(bool quiet)
{
  ...
  if (!quiet)
  {
      std::cout << "[tgModel::RemoveDublicateVertices] removed: " << rem << std::endl;
  }
}
...
void tgModel::DublicateCommonVertices()
{
  ...
  for (size_t i = 0; i < m_faces.size(); i++)
  {
    ...
    for (size_t j = 0; j < f.v.size(); j++)
    {
      ...
      if (vertex_used[vidx])
      {
        f.v[j] = (unsigned)m_vertices.size();
