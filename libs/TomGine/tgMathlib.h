...
#ifdef WIN32
#include <limits>
#define TG_NAN std::numeric_limits<float>::quiet_NaN();
#endif
...
inline vec2 param2polar(const vec2 &vp)
{
  ...
  if(v.x > epsilon){
    ...
    if(v.y > epsilon)
      ...
      r.x = 2.0f * float(M_PI) + atan(v.y/v.x);
    else // v.y == 0.0
      r.x = 0.0;
...
struct vec3 {
  ...
  inline bool operator!=(const vec3 &v) { return !(*this == v); }
...
inline vec3 rgb2hsv(const vec3 &rgb){
  ...
  if(fMax-fMin < 0.01f) h = 0.0f;
  else if(fMax == r) h = 60.0f * (0.0f + (g-b)/(fMax-fMin));
  else if(fMax == g) h = 60.0f * (2.0f + (b-r)/(fMax-fMin));
  else if(fMax == b) h = 60.0f * (4.0f + (r-b)/(fMax-fMin));

  if(h < 0.0f) h = h + 360.0f;

  if(fMax < 0.01f) s = 0.0f;
...
struct mat4 {
  ...
  void perspective(float fov,float aspect,float znear,float zfar) {
    ...
    float y = tan(fov * float(M_PI) / 360.0f);
    float x = y;
    if(aspect > 1)
      x *= aspect;
    else
...
struct quat {
  ...
  void set(const vec3 &dir,float angle) {
    ...
    if(length != 0.0) {
      length = 1.0f / length;
      float sinangle = sin(angle * (float(M_PI) / 180.0f) / 2.0f);
      x = dir[0] * length * sinangle;
      y = dir[1] * length * sinangle;
      z = dir[2] * length * sinangle;
      w = cos(angle * (float(M_PI) / 180.0f) / 2.0f);
    } else {
      x = y = z = 0.0;
      w = 1.0;
    }
