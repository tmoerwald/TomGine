                                         ...
                                         std::vector<GLint>& colorInternal, GLint depthInternal)
{
  ...
  {
    drawBuffers.push_back(GL_COLOR_ATTACHMENT0+(unsigned)i);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+(unsigned)i, GL_TEXTURE_2D, texColor[i]->GetTextureID(), 0);
  }
  tgCheckError("[tgFrameBufferObject::tgFrameBufferObject] attach color texture");
  ...
  glDrawBuffers((unsigned)drawBuffers.size(), &drawBuffers[0]);
