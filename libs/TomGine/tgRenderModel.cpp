  ...
  for(size_t i=0; i<vertex_indices.size(); i++)
  {
    ...
    if(vi<m_vertices.size())
    {
      ...
      if(tgCheckError("[tgRenderModel::UpdateBufferObjectVertices] B")!=GL_NO_ERROR)
      {
        ...
        std::cout << "[tgRenderModel::UpdateBufferObjectVertices] " << sizeof(tgVertex)*vi << " / " << s << std::endl;
        return;
      }
...
void tgRenderModel::DrawBufferObject(bool normals, bool texture)
{
  ...
  {
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_triangleIBO);
    glDrawElements(GL_TRIANGLES, (GLsizei)m_triangleIDX.size(), GL_UNSIGNED_INT, BUFFER_OFFSET(0));
  }

  if(!m_quadIDX.empty())
  {
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_quadIBO);
    glDrawElements(GL_QUADS, (GLsizei)m_quadIDX.size(), GL_UNSIGNED_INT, BUFFER_OFFSET(0));
  }
#ifdef DEBUG
...
void tgRenderModel::DrawNormals(float normal_length) const
{
  m_pose.Activate();
